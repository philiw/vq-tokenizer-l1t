...
def read_l1t_parquet_file(
    ...
    Each row in the file is one event with a variable-length list of AK8 jets.
    Events with no jets are filtered out. The jets are treated as the "particle"
    sequence (one token per jet).
    ...
    labels : list of str, optional
        Label field names to return. Defaults to ["label_l1t"].
    n_load : int, optional
        Maximum number of events to load after filtering. If None, all are loaded.
    shuffle_particles : bool, optional
        Unused (kept for signature compatibility).
    random_seed : int, optional
        Random seed for shuffling events within this file.
    ...
    ak_labels : ak.Array
        One-hot label array of shape (n_events,) with the requested label fields.
    """
    if labels is None:
        labels = ["label_l1t"]

    # Detect the jet type label from the filename
    from gabbro.utils.jet_types import jet_types_dict
    basename = Path(filepath).name
    label_value = 10  # default: l1t_minbias
    for jt_info in jet_types_dict.values():
        if jt_info.get("file_prefix") and basename.startswith(jt_info["file_prefix"]):
            label_value = jt_info["label"]
            break
    ...
    ak_labels = ak.Array({"label_l1t": np.full(n_events, label_value, dtype="float32")})

    return x_events, None, ak_labels[labels]
