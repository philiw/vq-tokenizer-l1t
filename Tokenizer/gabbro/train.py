...
def get_nodename_bigram():
    ...
    """
    nodename = os.uname().nodename if hasattr(os, "uname") else os.environ.get("COMPUTERNAME", "localhost")
...
def main(cfg: DictConfig) -> Optional[float]:
    ...
    if cfg.get("ckpt_path_for_evaluation") is not None:
        ...
        # Filter out defaults-list overrides (e.g. experiment=...) that don't apply
        # to the flat checkpoint config which has no defaults list.
        _task_overrides = [
            o for o in HydraConfig.get().overrides.task
            if not o.startswith("experiment")
        ]
        cfg = compose(config_name="cfg_ckpt", overrides=_task_overrides)
