...
"""
From: https://github.com/hqucms/weaver-core/blob/main/weaver/nn/model/ParticleTransformer.py
    ...
    def forward(self, x, v=None, mask=None, uu=None):
        ...
        if self.enabled:
            if self._counter < 5:
                ...
                if self.training:
                    q = min(1, random.uniform(*self.target))  # nosec
    ...
    def forward(self, x, uu=None):
        ...
        with torch.no_grad():
            ...
            if self.is_symmetric and not self.for_onnx:
                # print(f"x.shape before pairwise_lv_fts: {x.shape}")
                ...
                if x is not None:
                    ...
                    # print(f"x.shape after pairwise_lv_fts: {x.shape}")
                if uu is not None:
        ...
        if self.mode == "concat":
            ...
            if x is None:
                ...
                # print(f"elements.shape after embed: {elements.shape}")
            else:
...
class ParticleTransformerTaggerWithExtraPairFeatures(nn.Module):
    ...
    def forward(
        ...
        with torch.no_grad():
            ...
            mask = torch.cat([pf_mask, sv_mask], dim=2)
            uu = torch.zeros(
                v.size(0), pf_uu.size(1), v.size(2), v.size(2), dtype=v.dtype, device=v.device
            )
